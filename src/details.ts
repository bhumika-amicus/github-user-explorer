...
export async function initDetails(): Promise<void> {
    const statusEl = document.querySelector('#detail-status');
    const params = new URLSearchParams(window.location.search);
    const username = params.get('username');

    if (!username) {
        if (statusEl) statusEl.textContent = 'No username specified in URL.';
        return;
    }
    ...
    try {
        ...
        if (followersSettled.status === 'fulfilled') {
            ...
            if (followersRes.success) {
                ...
                const followersList = document.querySelector('#followers-list');
                if (followersList) followersList.textContent = 'Unable to load followers at this time.';
            }
        } else {
        ...
        if (reposSettled.status === 'fulfilled') {
            ...
            if (reposRes.success) {
                ...
                const reposList = document.querySelector('#repos-list');
                if (reposList) reposList.textContent = 'Unable to load repositories at this time.';
            }
        } else {
